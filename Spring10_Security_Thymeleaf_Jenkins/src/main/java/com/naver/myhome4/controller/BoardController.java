...
import jakarta.servlet.http.HttpServletResponse;
	...
	public class BoardController {
		...
		public String add(Board board, jakarta.servlet.http.HttpServletRequest request)
				throws Exception {
		...
		private String fileDBName(String fileName, String saveFolder) {
			...
			// (파일멸에 점에 여러개 있을 경우 맨 마지막에 발견되는 문자열의 위치를 리턴합니다.)
		...
		public ModelAndView BoardModifyView(
			...
			if (boarddata == null) {
				...
				// 수정 폼 페이지로 이동할 때 원문 글 내용을 보여주기 때문에 boarddata객체를
				// ModelAndView 객체에 저장합니다.
				mv.addObject("boarddata" , boarddata);
				// 글 수정 폼 페이지로 이동하기위해 설정합니다.
		...
		public String BoardModifyAction(
			...
			boolean usercheck =
					boardService.isBoarWriter(boarddata.getBOARD_NUM(), boarddata.getBOARD_PASS());
			String url="";
			// 비밀번호가 다른 경우
			if (usercheck == false) {
			...
			if (check != null && !check.equals("")) { // 기존파일 그대로 사용하는 경우입니다.
				...
				if (uploadfile!=null && !uploadfile.isEmpty()) {
					...
					//<input type="hidden" name="BOAED_FILE" value="${boarddata.BOARD_FILE}">
			...
			public ModelAndView BoardReplyAction(Board board, ModelAndView mv,
				...
				if (result == 0) {
					...
					//mv.setViewName("redirect:detail?num="+board.getBOAED_NUM());
			...
			public String BoardDeleteAction(String BOARD_PASS, int num,
				...
				boolean usercheck = boardService.isBoarWriter(num, BOARD_PASS);
